...
jest.mock('node-fetch', () => ({
  ...
  default: jest.fn().mockResolvedValue({
    ...
    json: async () => ({ urls: { regular: 'https://example.com/fallback.jpg' } })
  })
}));
...
describe('Agent Properties API', () => {
  ...
  describe('GET /api/agent/properties', () => {
    it('returns only properties for the authenticated agent', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/agent/properties');
    ...
    it('creates a new property for the authenticated agent', async () => {
      ...
      expect(prisma.property.create).toHaveBeenCalledWith({
        data: {
          ...mockProperty,
          agentId: 1,
          image: 'https://example.com/fallback.jpg' // matches your node‐fetch mock
        },
      });
    });
  ...
  describe('PATCH /api/agent/properties/[id]', () => {
    ...
    it('updates an existing property owned by the agent', async () => {
      ...
      (prisma.property.findUnique as jest.Mock).mockResolvedValue(existingProperty);
      (prisma.property.update as jest.Mock).mockResolvedValue({
        ...existingProperty,
        ...mockUpdate,
      });

      const request = new NextRequest('http://localhost:3000/api/agent/properties/1', {
        method: 'PATCH',
    ...
    it('returns 404 if property does not exist', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/agent/properties/999', {
        method: 'PATCH',
        body: JSON.stringify(mockUpdate),
      });
